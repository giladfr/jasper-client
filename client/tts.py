...
class IvonaLocalTTS(AbstractTTSEngine):
    """
    Uses the ivona local speech synthesizer
    Requires ivona binary to be installed locally
    """
    ...
    def is_available(cls):
        return (super(cls, cls).is_available() and
                diagnose.check_executable('ivona_tts'))
    ...
    def say(self, phrase):
        ...
        with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as f:
            fname = f.name
        cmd = ['ivona_tts', 'EN_us','Mark',phrase, fname]
        self._logger.debug('Executing %s', ' '.join([pipes.quote(arg)
                                                     for arg in cmd]))
        with tempfile.TemporaryFile() as f:
            subprocess.call(cmd, stdout=f, stderr=f)
    ...
    def play(self, filename):
        # FIXME: Use platform-independent audio-output here
        # See issue jasperproject/jasper-client#188
        cmd = ['aplay','-fS16_LE', str(filename)]
        self._logger.debug('Executing %s', ' '.join([pipes.quote(arg)
                                                     for arg in cmd]))
        subprocess.call(cmd)
...
if __name__ == '__main__':
    ...
    available_engines[5].get_instance().say("This is a test.")
